...
exports.api = exports.generateAllPages = exports.generatePage = void 0;
...
function generateCompleteHTML(cityData) {
    ...
    const highlightsHTML = cityData.highlights.map(highlight => `<div class="highlight-card">
      <p>${highlight}</p>
    </div>`).join('');
    const attractionsHTML = cityData.attractions.map(attraction => `<div class="attraction-card">
  ...
  ` : '';
    const faqsHTML = cityData.faqs ? cityData.faqs.map(faq => `<div class="faq-item">
...
async function extractCityDataFromTSX(tsxFilePath) {
    var _a;
    try {
        ...
        if (attractionsMatch) {
            ...
            while ((attractionMatch = attractionPattern.exec(attractionsContent)) !== null) {
                ...
                const interests = interestsMatch ?
                    ((_a = interestsMatch[1].match(/"([^"]+)"/g)) === null || _a === void 0 ? void 0 : _a.map(item => item.replace(/"/g, ''))) || [] : [];
                attractions.push({
                    name,
                    description,
                    practicalInfo: {
...
exports.generatePage = functions.https.onRequest(async (req, res) => {
    try {
...
exports.generateAllPages = functions.https.onRequest(async (req, res) => {
    try {
...
exports.api = functions.https.onRequest(async (req, res) => {
    ...
    if (path === '/generate-page') {
        return (0, exports.generatePage)(req, res);
    }
    else if (path === '/generate-all-pages') {
        return (0, exports.generateAllPages)(req, res);
    }
    else {
