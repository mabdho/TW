...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Generate all pages using Firebase HTML rendering system (public endpoint)
  app.post('/api/generate-all-pages-firebase', async (req, res) => {
    try {
      const results = [];
      const errors = [];
      ...
      try {
        ...
        errors.push({ page: 'Homepage', error: error instanceof Error ? error.message : 'Unknown error' });
      ...
      try {
        ...
        errors.push({ page: 'Destinations', error: error instanceof Error ? error.message : 'Unknown error' });
      ...
      try {
        ...
        errors.push({ page: 'Blogs', error: error instanceof Error ? error.message : 'Unknown error' });
      ...
      try {
        ...
        errors.push({ page: 'Legal Pages', error: error instanceof Error ? error.message : 'Unknown error' });
      ...
      try {
        ...
        errors.push({ page: 'City Pages', error: error instanceof Error ? error.message : 'Unknown error' });
      ...
      try {
        ...
        errors.push({ page: 'Individual Blog Pages', error: error instanceof Error ? error.message : 'Unknown error' });
      ...
      try {
        ...
        errors.push({ page: 'Sitemap', error: error instanceof Error ? error.message : 'Unknown error' });
      ...
      try {
        ...
        errors.push({ page: 'Robots.txt', error: error instanceof Error ? error.message : 'Unknown error' });
        console.error('  ❌ Robots.txt generation failed:', error);
      }
      
      // Summary
      console.log(`🎉 Firebase HTML generation completed!`);
      ...
      res.json({
        success: true,
        message: 'Firebase HTML generation completed',
        summary: {
      ...
      console.error('Error in comprehensive Firebase HTML generation:', error);
      res.status(500).json({
        error: 'Failed to generate all pages',
        details: error instanceof Error ? error.message : 'Unknown error'
      });
    }
  });
  ...
  // Universal HTML serving routes (same content for all users)
  // No user-agent discrimination - fully Google compliant
  
  // Serve home page HTML for all users
  app.get('/', async (req, res, next) => {
    try {
      ...
      // Continue to next middleware (React app) if HTML generation fails
      next();
    }
  });

  // Serve city page HTML for all users
...
function generateReactComponent(
  ...
  // Format gallery images with varied alt text to prevent keyword cannibalization
  const cityNameForAlt = cityName.replace(/([A-Z])/g, ' $1').trim();
  const altVariations = [
    `things to do in ${cityNameForAlt}`,
    `best places to visit in ${cityNameForAlt}`,
    `top attractions in ${cityNameForAlt}`,
    `must-see spots in ${cityNameForAlt}`,
    `destinations in ${cityNameForAlt}`
  ];
  const formattedGalleryImages = galleryImages.map((img, index) => `
        { url: "${img.url || ''}", alt: "${img.alt || altVariations[index % altVariations.length]}", caption: "${img.caption || ''}" }`).join(',');
